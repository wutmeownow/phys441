...
void gravrk(double x[], double t, double param[], double deriv[]) {
  ...
  double GM = param[1];
  double r1 = x[1], r2 = x[2];     
  double v1 = x[3], v2 = x[4];     
  double normR = sqrt( r1*r1 + r2*r2 );
  double accel1 = -GM*r1/(normR*normR*normR);  
...
void graph_orbit() {
  ...
  for( iStep=1; iStep<=nStep; iStep++ ) {  
    ...
    tplot[iStep] = time;
    kinetic[iStep] = 0.5*mass*normV*normV;   
