...
Double_t fitf(Double_t *v, Double_t *par){
	Double_t fit = par[0]*v[0]*v[0]*(exp(-par[1]*v[0]*v[0]));
	return fit;
}
...
void dsmceq() {
	...
	tottime.Stop();
	cout << "Total time Real: " << tottime.RealTime() << " CPU: " << tottime.CpuTime() << endl;
	cout << "Sorter time Real: " << sorttime.RealTime() << " CPU: " << sorttime.CpuTime() << endl;
	cout << "Colider time Real: " << coltime.RealTime() << " CPU: " << coltime.CpuTime() << endl;
	cout << "Expected Mean: " << sqrt(8*boltz*T/(pi*mass)) << endl;
	cout << "Actual Mean: " << hvf->GetMean() << endl;
	cout << "Expected RMS: " << sqrt((3*boltz*T)/mass) << endl;
	cout << "Actual RMS: " << hvf->GetRMS() << endl;
   //*-------------------------------GRAPHING------------------------------	
   gStyle->SetOptFit(1);	
   TCanvas *c1 = new TCanvas("c1","Data",200,10,700,500);
	TF1 *f1 = new TF1("f1", fitf, 0, 1000,2);
	Double_t param1 = sqrt(pow(mass/(2*pi*boltz*T),3))*4*pi;
	cout << "Param1: " << param1 << endl;
	Double_t param2 = mass/(2*boltz*T);
	cout << "Param2: " << param2 << endl;
	f1->SetParameters(param1,param2);
   c1->Divide(2,1);
   c1->SetFillColor(42);
   c1->SetGrid();
	c1->cd(1);
