...
void dsmceq() {
 ...
 TH1D *hvi = new TH1D("hvi", "test", 75, 0, 1000);
  ...
  double parameter1=sqrt(pow(((mass)/(2*3.14159*boltz*T)),3));
  double parameter2=((mass)/(2*boltz*T));
   ...
   TF1 *fitFunction = new TF1("Maxwell-Boltzmann Fit",fitf,0,1000,2);
   fitFunction->SetParameters(parameter1,parameter2);
   c1->Divide(2,1);
   c1->SetFillColor(42);
   c1->SetGrid();
	c1->cd(1);
...
{
  Double_t fit =par[0]*x[0]*x[0]*(exp(-par[1]*x[0]*x[0]));
  return fit;
}
