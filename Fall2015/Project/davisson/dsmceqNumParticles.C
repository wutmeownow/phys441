...
void dsmceqNumParticles() {
  ...
  Double_t *total = new Double_t[numTemps];
  Double_t *colider = new Double_t[numTemps];
  Double_t *sorter = new Double_t[numTemps];
  ...
  //cin >> T;
   ...
   for(npart=20;npart<1000;npart=npart+20)
    {
      totalTime->Start();
      temperatures[counter]=npart;
      cout<<"Number of particles is at: "<<npart<<endl;
 ...
 TH1D *hvi = new TH1D("hvi", "test", 75, 0, 1000);
  ...
  double MBrms = sqrt((3*boltz*T)/(mass));
      ...
      colider[counter]=coliderTime->RealTime();
      sorter[counter]=sorterTime->RealTime();
      total[counter]=totalTime->RealTime();
   ...
   //c1->cd(3);
	...
	gr->SetTitle("Number of Particles vs Number of Steps");
	gr->SetMarkerStyle(22);
	gr->GetXaxis()->SetTitle("Number of Particles");
	gr->GetYaxis()->SetTitle("Number of Steps"); 
	c1->cd(1);
	gr->Draw("ALP");

	TGraph *gr2=new TGraph(numTemps,temperatures,total);
	gr2->SetTitle("Number of Particles vs Total Time");
	gr2->SetMarkerStyle(22);
	gr2->GetXaxis()->SetTitle("Number of Particles");
	gr2->GetYaxis()->SetTitle("Total Time"); 
	c1->cd(2);
	gr2->Draw("ALP");

	TGraph *gr3=new TGraph(numTemps,temperatures,colider);
	gr3->SetTitle("Number of Particles vs Colider Time");
	gr3->SetMarkerStyle(22);
	gr3->GetXaxis()->SetTitle("Number of Particles");
	gr3->GetYaxis()->SetTitle("Colider Time"); 
	c1->cd(3);
	gr3->Draw("ALP");

	TGraph *gr4=new TGraph(numTemps,temperatures,sorter);
	gr4->SetTitle("Number of Particles vs Sorter Time");
	gr4->SetMarkerStyle(22);
	gr4->GetXaxis()->SetTitle("Number of Particles");
	gr4->GetYaxis()->SetTitle("Sorter Time"); 
	c1->cd(4);
	gr4->Draw("ALP");
