...
void graph_pendulum(Double_t length=9.807) {
	...
	double area = 0.007854;  // Cross-sectional area of projectile (m^2)
	...
	for( iStep=1; iStep<=nStep; iStep++ ) {  
		...
		double Normomega = sqrt(omega*omega);
		//* Compute new position and velocity using 
		//    Euler or Verlet method
		accel = (air_const*Normomega*omega)- g_over_L*sin(theta);    // acceleration
		if( method == 1 ) {
