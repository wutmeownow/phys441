...
void graph_projectile() {
	...
	//Modified: Samuel J. Porter -> added bounce, user sets restitution coefficients
	//		
	...
	double *vyplot;
	vyplot = new double [maxStep+1];

	// Set restitution coefficients
	double Cry, Crx;
	cout << "Enter coef of restitution for y (between 0 and 1) "; cin >> Cry;
	cout << "Enter coef of restitution for x (between 0 and 1) "; cin >> Crx;
	if(Cry <= 0 || Cry >= 1){
		cout << "The value for the y resititution is out of bounds, value set to 0.3" << endl;
		Cry = 0.3;
	}
	if(Crx <= 0 || Crx >= 1){
		cout << "The value for the x resititution is out of bounds, value set to 0.9" << endl;
		Crx = 0.9;
	}
	...
	for( iStep=1; iStep<=maxStep; iStep++ ) {
		...
		//* If ball reaches ground (y<0), bounce
		if( r[2] < 0 )  {
			//cout << "hit at x = " << r[1] << "," << r[2] << "," << t << endl;
			//cout << "v[2] before hit: " << v[2] << endl;
			//cout << "normV: " << normV << endl;
			v[1] = (v[1]*Crx);
			v[2] = -(v[2]*Cry);
			r[2] = -r[2];
			//cout << "v[2] after hit: " << v[2] << endl;
			//cout << "normV: " << normV << endl;
			//cout << endl;

		} 
		//* When total velocity of ball drops below threshold, break out of loop
		if(normV <= 0.5) {
			/*cout << endl;
			cout << "v[1]: " << v[1] << endl;
			cout << "v[2]: " << v[2] << endl;
