...
void graph_projectile() {
	...
	//* Loop until ball stops or max steps completed
	double tau;
	...
	for( iStep=1; iStep<=maxStep; iStep++ ) {
		...
		if( r[2] < 0 )  {
			r[2] = 0;
			double totalv = pow(pow(v[1],2)*pow(v[2],2),0.5);
			if(totalv > 0.5){
				//Compute for bounces
				v[1] = 0.9 * v[1];
				v[2] = -0.3 * v[2];
	...
	cout << endl << "Maximum range is " << r[1] << " meters" << endl;
	cout << "Time of movement is " << iStep*tau << " seconds" << endl;
