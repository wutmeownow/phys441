...
void graph_pend_air(Double_t length=9.807) {
   //
  ...
  double mass = 1.0;
  double radius = 0.1;
  double Cd = 0.47;      // Drag coefficient (dimensionless)
  ...
  for( iStep=1; iStep<=nStep; iStep++ ) {  
    ...
    accel = -g_over_L*sin(theta)+air_const*sqrt(omega*omega)*omega*length;
    if( method == 1 ) {
