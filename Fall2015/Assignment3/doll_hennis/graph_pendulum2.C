...
void graph_pendulum2(Double_t length=9.807) {
   //
  ...
  double area = .00314159;  // Cross-sectional area of projectile (m^2)
  double grav = 9.81;    // Gravitational acceleration (m/s^2)
  double mass = 1.0;   // Mass of projectile (kg)
  double air_const = -0.5*Cd*1.2*area/mass;  // Air resistance constant

  //* Take one backward step to start Verlet
  double accel = -sin(theta);    // Gravitational acceleration
  ...
  int nStep;  cin >> nStep;
  double *t_plot, *th_plot, *period, *trev, *O_plot;  // Plotting variables
