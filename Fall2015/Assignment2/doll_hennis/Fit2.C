...
#include "TMatrixD.h"
#include "TRandom.h"
//Edited by Austin Doll-Hennis to answer questions 5 and 6
...
void Fit2() {
   ...
   const Int_t m = 7;
   Double_t x[m], y[m], ex[m],ey[m], dx[m], dy[m];
   Double_t xvalue[1];
...
//draw graph with error bars
   gr = new TGraphErrors(m,x,y,dx,dy);
   TF1 *pfit1 = new TF1("fitfunction",fitfunction,0.0,4.0,2);
   pfit1->SetParameters(0.01,0.01);
