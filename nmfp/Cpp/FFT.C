...
//********* Histograms ********//
   ...
   //TF1 *fsin = new TF1("fsin", "0.01*sin(2*x*2.0*TMath::Pi())+0.005*sin(5*x*2.0*TMath::Pi())+0.002*sin(10.0*x*2*TMath::Pi())", 0, 1);
   TF1 *fsin = new TF1("fsin", "0.01*cos(2*x*2.0*TMath::Pi())+0.005*cos(5*x*2.0*TMath::Pi())+0.002*cos(10.0*x*2*TMath::Pi())", 0, 1);
   TRandom3 *r=new TRandom3();
   
   Int_t n=100;
   TH1D *hsin = new TH1D("hsin", "hsin", n, 0.0, 1.0);
   Double_t x,offset;

   Int_t npoints=10000000;
   ...
   const Int_t nin_bins = 2*((n+1)/2+1);
   cout << "Number of bins = " << nin_bins << endl;

   Double_t *in = new Double_t[nin_bins];
   for (Int_t i=0; i<n; i++){
      x = (Double_t(i)/n);
      offset = hsin->GetBinContent(i+1);
      cout << "i= " << i << " content = " << offset << endl;
      in[i+1]=fsin->Eval(x)+offset;
