...
//
   ...
   for(i=0;i<n;i++){
	   xvalue[0]=x[i];
	   yfit[i]=fitfunction(xvalue,parameter_matrix);
	   diff[i]=y[i]-yfit[i];
   }


   gr2 = new TGraphErrors(n,x,diff);
//   gr2->SetLineColor(2);
//   gr2->SetLineWidth(4);
//   gr2->SetMarkerColor(4);
   gr2->SetMarkerStyle(21);
   gr2->SetTitle("TV Ownership Rate (Actual - Fit)");
   gr2->GetXaxis()->SetTitle("Year");
   gr2->GetYaxis()->SetTitle("Rate");
//
//   TAxis *axis = gr->GetXaxis();
//   axis->SetLimits(-5.,5.);
   gr2->GetHistogram()->SetMaximum(5.0);
