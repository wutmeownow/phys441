...
void graph_projectile_tau_comparison() {
  ...
  double rc[2+1], vc[2+1], accelc[2+1]; 
  y1=2.0;
  ...
  v[1] = v1[1];  v[2] = v1[2];             
  rc[1] = r1[1];  rc[2] = r1[2];  // Set initial position and velocity
  vc[1] = v1[1];  vc[2] = v1[2];             
  ...
  for( iStep=1; iStep<=maxStep; iStep++ ) {
    ...
    double normVc = sqrt( vc[1]*vc[1] + vc[2]*vc[2] );
    accelc[1] = air_const*normVc*vc[1];   // Air resistance
    accelc[2] = air_const*normVc*vc[2]-grav;   // Air resistance - Gravity
  
    //* Calculate the new position and velocity using Euler method
    r[1] += tau*v[1];                 		// Euler step
    r[2] += tau*v[2];                 
    rc[1] += tauc*vc[1];                 		// Euler step
    rc[2] += tauc*vc[2];                 
  ...
  cout << "Max y = " << max_y << endl; 
   ...
   gxtt = new TGraph(n,timec,xNoAir);
   gyt = new TGraph(n,time,yplot);
   gytt = new TGraph(n,timec,yNoAir);
