...
void graph_traffic_tunnel(double factor=0.25, double alpha = 0.20) {
  ...
  for( i=0; i<N; i++ ) {
	  ...
	  double t1=alpha*exp(-(x-x1)*(x-x1)/(2.0*sigma*sigma));
	  double t2=alpha*exp(-(x-x2)*(x-x2)/(2.0*sigma*sigma));
	  double t3=alpha*exp(-(x-x3)*(x-x3)/(2.0*sigma*sigma));
	  double t4=alpha*exp(-(x-x4)*(x-x4)/(2.0*sigma*sigma));
	  double t5=3.0*alpha*exp(-(x-x5)*(x-x5)/(2.0*(2.0*sigma)*(2.0*sigma)));
	  rho[i]=rho0*(1+(t1+t2+t3+t4+t5));
	  rho_start[i]=rho[i];
  }
