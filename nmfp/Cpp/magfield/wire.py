...
class Wire:
    ...
    def IdL_r1(self):
        ...
        if npts < 2:
            ...
            print("length of discretized path = ",npts)
            #print("discretized path = ",self.discretized_path)
    ...
    @staticmethod
    def LinearPath(pt1=(0.5, 0.5, 0.0), pt2=(0.5, 0.5, 1.0), pts = 100):
        t = np.linspace(0, 1, pts)
        return np.array([pt1[0]+(pt2[0]-pt1[0])*t,pt1[1]+(pt2[1]-pt1[1])*t,pt1[2]+(pt2[2]-pt1[2])*t]).T
    ...
    @staticmethod
    def CircularPath(radius=0.5, pts=20):
        t = np.linspace(0, 2 * np.pi, pts)
        result=np.array([radius * np.sin(t), radius * np.cos(t), 0.0]).T
        return result

    @staticmethod
    def SinusoidalCircularPath(radius=0.5, amplitude=0.0, frequency=10, pts=20):
        t = np.linspace(0, 2 * np.pi, pts)
    ...
    def EllipticalPath(rx=0.1, ry=0.2, pts=20):
        t = np.linspace(0, 2 * np.pi, pts)
        result=np.array([rx * np.sin(t), ry * np.cos(t), 0.0]).T
        return result
