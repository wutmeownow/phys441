...
void graph_integration_multiple_3D(Int_t max_order = 3, Int_t nbins = 20, Int_t min_order = 2) {
  ...
  //* Initialize the polynomial
	   ...
	   double dy = (yhigh-ylow)/npoints;
	   double dz = (zhigh-zlow)/npoints;
	   
	   sum[j-min_order][k] = 0.0;
   	   double xval, yval, zval, tval, tval_tempx, tval_tempy;
	   cout << "Starting i,l,m loop for bin = " << k << endl;
	   for (int i = 0; i<npoints;i++){
		   xval = rnum->Uniform(xlow,xhigh);
		   tval_tempx=myfunction(xval);
		   for (int l = 0; l<npoints;l++){
			   yval = rnum->Uniform(ylow,yhigh);
			   tval_tempy = tval_tempx*myfunction(yval);
			   for (int m=0; m<npoints; l++){
				zval = rnum->Uniform(zlow,zhigh);
		     		tval = tval_tempy*myfunction(zval);		
		   	   	sum[j-min_order][k]+= dx*dy*dz*tval;
			   }
	   	   }
	   }	   
