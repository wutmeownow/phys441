...
void graph_fitting_nonlinear() {
   ...
   while (dbetamax > epsilon){
   	for (i=0;i<m;i++){
        ...
        for (i=0;i<m;i++){cout << residuals[i][0] << endl;}
   	...
   	TMatrixD JTr(JacobianT,TMatrixD::kMult,residuals);
        cout << "Jacobian Transposed * Resid:" << endl;
        for (i=0;i<n;i++){
            for (j=0;j<1;j++){
                cout << JTr[i][j] << "   ";
            }
   	...
   	TMatrixD JTJ(JacobianT,TMatrixD::kMult,Jacobian);
        /*cout << "Jacobian Transposed * Jacobian:" << endl;
        for (i=0;i<n;i++){
            for (j=0;j<n;j++){
                cout << JTJ[i][j] << "   ";
            }
   	...
   	JTJ.InvertFast(&det);
        cout << "Inverse Jacobian Transposed * Jacobian:" << endl;
        for (i=0;i<n;i++){
            for (j=0;j<n;j++){
                cout << JTJ[i][j] << "   ";
                jtjarray[i][j]=JTJ[i][j];
            }
            cout << endl;
        }
   ...
   TH3D *h3 = new TH3D("h3","S-squared Histogram",nbins,xlow,xhigh,nbins,ylow,yhigh,nbins,0.0,zmax);
   for (j=0; j<nbins; j++){
...
//   printf("Fit 1: %f %f \n",pfit1chi2,pfit1ndf);
   ...
   for (k=0;k<n;k++){
	   cout << "beta" << k+1;
	   for (i=0;i<n;i++){
   	                cout << "    " << chi2*jtjarray[k][i];
	   }
	   cout << endl;
           error[k]=pow(chi2*jtjarray[k][k],0.5);
   }
