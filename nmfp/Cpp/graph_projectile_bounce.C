...
void graph_projectile_bounce() {
  ...
  double coeff_rest_x = 0.95;
  double coeff_rest_y = 0.50;
  ...
  for( iStep=1; iStep<=maxStep; iStep++ ) {
    ...
    double tg = t - tNoAirOffset;
    xNoAir[iStep] = r1[1] + v1[1]*tg;
    yNoAir[iStep] = r1[2] + v1[2]*tg - 0.5*grav*tg*tg;
    if(yNoAir[iStep] < 0.001){
	    cout << "Bounce:  x = " << xNoAir[iStep] << " y = " << yNoAir[iStep] << " vx = " << v1[1] << " vy = " << (v1[2]-grav*tg) << endl;
	    //cout << "x = " << xNoAir[iStep] << " y = " << yNoAir[iStep] << endl;
	    v1[1]=coeff_rest_x*v1[1];
	    v1[2]=-1.0*coeff_rest_y*(v1[2]-grav*tg);
	    r1[2]=0.001;
	    r1[1]=xNoAir[iStep];
	    tNoAirOffset=t;
	    cout << "Restart:  x = " << r1[1] << " y = " << r1[2] << " vx = " << v1[1] << " vy = " << v1[2] << endl;
    }
    ...
    //* If ball reaches ground (y<0.001), create a "bounce"
    if( r[2] < 0.001 )  {
  ...
  cout << "Max y = " << max_y << endl; 
   ...
   gStyle->SetOptFit(1);	
   TCanvas *c1 = new TCanvas("c1","XY Projectile",200,10,700,700);
   c1->SetFillColor(42);
   c1->SetGrid();
   TCanvas *c2 = new TCanvas("c2","XT Projectile",200,10,700,700);
   c2->SetFillColor(42);
   c2->SetGrid();
   TCanvas *c3 = new TCanvas("c3","YT Projectile",200,10,700,700);
   c3->SetFillColor(42);
   c3->SetGrid();
