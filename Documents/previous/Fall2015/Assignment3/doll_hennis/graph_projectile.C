...
void graph_projectile() {
  ...
  double cRestX = -.30;    /////coefficient of restitution in x direction
  double cRestY = .90;    /////restitution in y direction
  bool bouncer = false; 
  ...
  //* Loop until velocity is less than .5 m/s or max steps completed
  double tau;
  ...
  if(bouncer == false){
    ...
    //* If ball reaches ground (y<0), instead of breaking out of the loop,
	//make the ball bounce until the velocity is less than .5 m/s
    if( r[2] < 0 )  {
	  bouncer = true;
	}
...
//guess
    ...
    // if the velocity falls too low, break the loop
    	if( normV < .5 ))  {
