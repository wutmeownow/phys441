...
void part6() {
   ...
   x[0]=0.038; y[0]=0.050;  ey[0]=0.01;
   x[1]=0.194; y[1]=0.127;  ey[1]=0.01;
   x[2]=0.425; y[2]=0.094;  ey[2]=0.03;
   x[3]=0.626; y[3]=0.2122; ey[3]=0.03;
   x[4]=1.253; y[4]=0.2729; ey[4]=0.03;
   x[5]=2.500; y[5]=0.2665; ey[5]=0.02;
   x[6]=3.740; y[6]=0.3317; ey[6]=0.01;
   ...
   while (dbetamax > epsilon){
   	for (i=0;i<m;i++){
	   	...
	   	residuals[i][0]=(y[i]-function_value)/ey[i];
			for (j=0;j<n;j++){
				if (j==0){	
					jacobian = jacobian_beta1(xvalue,parameter_matrix);
		       	Jacobian[i][j]=jacobian/ey[i];
				}else{
   ...
   for (j=0; j<m; j++){
	   ...
	   error_sum = error_sum + pow((yfit-y[j]),2)/pow(ey[j],2.0);
	   yfit = 0;
   }
   ...
   cout << endl << endl;
   cout << "The Information I Found: " << endl;
   cout << " chi^2          = " << chi2*(m-n) << endl;
   cout << " NDf            = " << (m-n) << endl;
   for (k=0;k<n;k++){
      error[k]=pow(JTJ[k][k],0.5);
