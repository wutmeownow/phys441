...
void stepvstemp() {
	...
	//cin >> T;
	Double_t* temps = new Double_t[150];
	Double_t* steps = new Double_t[150];
	for (T= 20; T<=3000; T+=20){ 
		cout << "Temperature (in Kelvin): " << T << endl;
		int noneq = 0;
   ...
   c1->SetFillColor(42);
   c1->SetGrid();
	TGraph* g = new TGraph(150,temps,steps);
   g->SetMarkerStyle(22);
   g->SetTitle("Steps vs Temps");
