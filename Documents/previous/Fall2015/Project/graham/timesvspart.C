...
void timesvspart() {
	...
	//cin >> T;
	Double_t* part = new Double_t[20];
	Double_t* tottimes = new Double_t[20];
	Double_t* coltimes = new Double_t[20];
	Double_t* sorttimes = new Double_t[20];
	for (npart= 100; npart<=2000; npart+=100){ 
		...
		for( int istep = 1; istep<=nstep; istep++ ) {
			...
			sorttime.Start(kFALSE);
			//* Sort the particles into cells
			sorter(x,L,sortData);
			sorttime.Stop();
			//* Evaluate collisions among the particles
			coltime.Start(kFALSE);
			...
			for( i=1; i<=npart; i++ ){
				...
				if (vmagF[i][1] == vmagI[i][1]) noneq++;
			}
			if (noneq <= npart*0.05) break;
			else noneq = 0;
		}
		tottime.Stop();
		tottimes[(npart/100)-1] = tottime.RealTime();
		coltimes[(npart/100)-1] = coltime.RealTime();
		sorttimes[(npart/100)-1] = sorttime.RealTime();
		part[(npart/100)-1] = npart;
		coltime.Reset();
		sorttime.Reset();
	}
   ...
   c1->SetFillColor(42);
   c1->SetGrid();
	TGraph* g1 = new TGraph(20,part,tottimes);
   g1->SetMarkerStyle(22);
   g1->SetTitle("Total Time vs Particles");
   g1->GetXaxis()->SetTitle("Particles");
   g1->GetYaxis()->SetTitle("Total Time");
	TGraph* g2 = new TGraph(20,part,coltimes);
   g2->SetMarkerStyle(22);
   g2->SetTitle("Colider Time vs Particles");
   g2->GetXaxis()->SetTitle("Particles");
   g2->GetYaxis()->SetTitle("Colider Time");
	TGraph* g3 = new TGraph(20,part,sorttimes);
   g3->SetMarkerStyle(22);
   g3->SetTitle("Sorter Time vs Particles");
   g3->GetXaxis()->SetTitle("Particles");
   g3->GetYaxis()->SetTitle("Sorter Time");
	c1->cd(1);
   g1->Draw("ALP");
	c1->cd(2);
	g2->Draw("ALP");
