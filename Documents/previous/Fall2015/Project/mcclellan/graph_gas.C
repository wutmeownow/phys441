...
int colider( TMatrixD& v, TMatrixD& crmax, double tau, TMatrixD& selxtra, double coeff, SortList& sD ) {
...
//    col       Total number of collisions processed    (Return value)
  ...
  for( jcell=0; jcell<ncell; jcell++ ) {
    ...
    double select = coeff*number*(number-1)*crmax[jcell][0] + selxtra[jcell][0];
    int nsel = (int)(select);      // Number of pairs to be selected
    selxtra[jcell][0] = select-nsel;  // Carry over any left-over fraction
    double crm = crmax[jcell][0];     // Current maximum relative speed
    ...
    for( isel=0; isel<nsel; isel++ ) {
      ...
      double cr = sqrt( pow(v[ip1][0]-v[ip2][0],2) +
