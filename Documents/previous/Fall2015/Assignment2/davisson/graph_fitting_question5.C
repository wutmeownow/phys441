...
void graph_fitting_question5(int functionOrder=1) {
   //
...
*/
   ...
   cout<<"Chi value ["<<attempt<<"] "<<allChiSquared[attempt]<<endl;
   // cout<<y[attempt]<<endl;
   //cout<<yError[attempt]<<endl;
   allChiSquared[attempt]/=(n-m-1);
    cout<<"Chi value per degree of freedom ["<<attempt<<"] "<<allChiSquared[attempt]<<endl;
 }
